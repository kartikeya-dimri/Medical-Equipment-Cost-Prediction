...
# --- 3. Create Pipeline ---
# This pipeline combines polynomial features with Lasso regression
pipeline = Pipeline([
    ('poly', PolynomialFeatures(degree=BEST_DEGREE_FROM_PREVIOUS_STEP, include_bias=False)),
    ('lasso', Lasso(max_iter=10000, random_state=42)) # 'lasso' is the step name
])

# --- 4. Define Hyperparameter Grid for Tuning ---
# We will tune the 'alpha' parameter of the 'lasso' step
# A different range of alphas is often good for Lasso vs. Ridge
param_grid = {
...
print("\n--- Running GridSearchCV to find best alpha for Lasso ---")
...
submission_path = os.path.join(OUTPUT_DIR, 'lasso_regression_tuned.csv')
