...
# (I've set it to 2 as a placeholder).
#
BEST_DEGREE_FROM_PREVIOUS_STEP = 1
#
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# --- 3. Create Pipeline ---
# This pipeline combines polynomial features with Ridge regression
pipeline = Pipeline([
    ('poly', PolynomialFeatures(degree=BEST_DEGREE_FROM_PREVIOUS_STEP, include_bias=False)),
    ('ridge', Ridge(max_iter=2000)) # 'ridge' is the name of this step
])

# --- 4. Define Hyperparameter Grid for Tuning ---
# We will tune the 'alpha' parameter of the 'ridge' step
# Note the syntax: 'step_name__parameter_name'
param_grid = {
    'ridge__alpha': [0.01, 0.1, 1.0, 5.0, 10.0, 100.0] 
    # You can add more values to this list if you want to search more
}

# --- 5. Set up Grid Search with k=5 CV ---
kf = KFold(n_splits=5, shuffle=True, random_state=42)

# GridSearchCV will test every alpha in param_grid using 5-fold CV
# It will find the one that minimizes the RMSE
...
print("\n--- Running GridSearchCV to find best alpha for Ridge ---")
...
# The 'grid_search' object is now the best model, automatically
# refitted on the entire training set
...
submission_path = os.path.join(OUTPUT_DIR, 'ridge_regression_tuned.csv')
