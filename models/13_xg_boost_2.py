...
# This is a small grid; XGBoost can be very slow to tune
param_grid = {
    'n_estimators': [100, 300],
    'learning_rate': [0.1, 0.05],
    'max_depth': [3, 5],
    'colsample_bytree': [0.8, 1.0], # Fraction of features to use per tree
    'subsample': [0.8, 1.0]         # Fraction of samples to use per tree
}
...
print("\n--- Running GridSearchCV for XGBoost (v2 data) ---")
print(f"Searching {len(param_grid['n_estimators']) * len(param_grid['learning_rate']) * len(param_grid['max_depth']) * len(param_grid['colsample_bytree']) * len(param_grid['subsample'])} combinations...")
...
submission_path = os.path.join(OUTPUT_DIR, 'xgboost_grid_2.csv')
