...
# --- 1. Define Paths and Load FOURTH Data (v4) ---
...
print("Loading preprocessed data (v4)...")
try:
    # Load the new files from preprocessing4.py
    X_train = np.load(os.path.join(OUTPUT_DIR, 'X_train_processed4.npy'))
    y_train = np.load(os.path.join(OUTPUT_DIR, 'y_train_processed4.npy'))
    X_test = np.load(os.path.join(OUTPUT_DIR, 'X_test_processed4.npy'))
    
    df_test = pd.read_csv(os.path.join(DATA_DIR, 'test.csv'))
    hospital_ids = df_test['Hospital_Id']

except FileNotFoundError:
    print(f"Error: Could not find '...processed4.npy' files in '{OUTPUT_DIR}'.")
    print("Please run 'preprocessing/preprocessing4.py' first.")
    sys.exit(1)
...
# This is a small grid but will still be slow to run.
param_grid = {
...
# You can change n_splits to 5 if you are willing to wait longer.
...
print("\n--- Running GridSearchCV for Random Forest (v4 data) ---")
...
submission_path = os.path.join(OUTPUT_DIR, 'random_forest_tuned_4.csv')
